...
System.out.println(numbers); // Содержит 1, 2, 3; порядок HashSet не гарантирован.
```

### Map (Карта)
Карта хранит пары «ключ — значение», где каждый ключ уникален. `Map` входит в Collections Framework, но не наследует интерфейс `Collection`.
...
### Map в заводском складе

Для склада используем `Map<Resource, Integer>`: ключ — вид ресурса, значение — количество. У обобщённых типов в угловых скобках нужны ссылочные типы, поэтому здесь `Integer`, а не `int`. Java автоматически упаковывает и распаковывает целые значения; распаковка `null` вызвала бы ошибку.

В готовом `Inventory` используются такие операции (фрагмент внутри его метода):

```java
int ore = amounts.getOrDefault(Resource.ORE, 0);
amounts.put(Resource.ORE, ore + 1);
int total = 0;
for (int count : amounts.values()) {
    total += count;
}
```

`getOrDefault` возвращает указанное значение, если ключа нет. `put` добавляет запись или заменяет прежнее значение. `values()` позволяет перебрать количества циклом for-each. Пример показывает API; в настоящем складе изменение выполняется после проверки вместимости.

`EnumMap` подходит для enum-ключей; перечисления разбираются в курсе завода. `Map.of` создаёт немодифицируемый каталог, а `Map.copyOf` — немодифицируемую копию записей. Подробнее: [Oracle: Map, Java 21](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/Map.html).

Для первой части завода достаточно этих операций. Следующие разделы со Stream API и сортировкой пока можно пропустить.
...
- **LinkedList** - двусвязный список; поиск позиции требует обхода, поэтому частые вставки сами по себе не делают его лучшим выбором
